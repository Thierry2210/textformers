  ...
  <header class="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-primary-200/50 shadow-soft">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-16">
        ...
        <button @click="isOpen = !isOpen" class="sm:hidden p-2 rounded-lg text-secondary-700 hover:bg-primary-50 transition-colors duration-200">
        ...
        <nav class="hidden sm:flex items-center gap-1">
           ...
           <router-link v-if="isLoggedIn && isAdmin" :class="navClass('/admin')" to="/admin">Admin</router-link>
         </nav>
    ...
    <div v-if="isOpen" ref="menuRef" class="sm:hidden border-t border-primary-200/50 bg-white/95 backdrop-blur-md">
      <div class="max-w-7xl mx-auto px-4 py-4">
        <nav class="flex flex-col gap-1">
           ...
           <router-link v-if="isLoggedIn" @click="isOpen = false" :class="mobileNavClass('/essay')"
             to="/essay">Redação</router-link>
           <router-link v-if="isLoggedIn" @click="isOpen = false" :class="mobileNavClass('/plans')"
             to="/plans">Planos</router-link>
           <router-link v-if="isLoggedIn" @click="isOpen = false" :class="mobileNavClass('/history')"
             to="/history">Histórico</router-link>
           <router-link v-if="isAdmin" @click="isOpen = false" :class="mobileNavClass('/admin')"
